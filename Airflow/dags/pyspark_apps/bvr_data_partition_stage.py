...
def aggregate_s3_bucket(spark,params,day_select,record_name,key_content_data,schema_type):
    ...
    Args:
        spark (object): spark application 
        params (json): parameters to be loaded from AWS SSM
        day_select (str): day that needs to be processed
        record_name (str): data file name
        key_content_data (list): keys of the s3 bucket 
        schema_type (str):name of schema

    Returns:
        spark dataframe: aggregated data in spark dataframe output
    """
...
def process_sensor_data(spark,sensor_data, day_select, params):
    ...
    Args:
        spark (object ): spark application
        sensor_data (spark dataframe): data extracted from s3 bucket 
        day_select (str): day to be processed
        params (json): parameters to read from AWS SSM
    """
    ...
    sensor_data_table.write.mode('append').partitionBy("axleCount", "axle_index").parquet(f"s3a://{params['processed_data']}/partitioned-bvr-data/{day_select}", mode="append")
...
def main():
    ...
    key_content_data=get_s3_bucket(params["bvr_data"])

    sensor_data=aggregate_s3_bucket(spark,params,day_select,record_name,key_content_data,schema_type)
    sensor_data_=sensor_data.dropDuplicates((["bvrId","axleCount","patch_weight"])).select(sensor_data.columns)
    logging.info("DAY {} NO DUPLICATE DATA rows= {} and columns={}".format(day_select,sensor_data_.count(),len(sensor_data_.columns)))

    print("DAY {} NO DUPLICATE DATA rows= {} and columns={}".format(day_select,sensor_data_.count(),len(sensor_data_.columns)))
